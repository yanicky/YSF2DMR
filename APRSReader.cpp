...
if (lon_grad>=0 && lon_grad<=9) {a=0x50;b=(lon_grad+0x76U);}
else if (lon_grad>=10 && lon_grad<=99) {a=0x30;b=((lon_grad-10U)+0x26U);}
else if (lon_grad>=100 && lon_grad<=109) {a=0x50;b=((lon_grad-100U)+0x6CU);}
else {a=0x50;b=((lon_grad-110U)+0x26U);}
...
if (long_min>=0 && long_min<=9) b=long_min+0x58U;
else b=(long_min-10U)+0x26;
*(buffer+12U)=b;
...
bool CAPRSReader::findCall(std::string cs, int *latitude, int *longitude)
{
	...
	if (m_new_callsign) return false;
